...
func TestRun(t *testing.T) {
	files, err := os.ReadDir("./ascii-mountains")
	...
	for _, file := range files {
		...
		fileTask, err := os.Open(fmt.Sprintf("ascii-mountains/%s", file.Name()))
		require.NoError(t, err)
		...
		t.Run(fmt.Sprintf("Test:%s", file.Name()), func(t *testing.T) {
			...
			expected, err := os.ReadFile(fmt.Sprintf("ascii-mountains/%s.a", file.Name()))
			require.NoError(t, err)
