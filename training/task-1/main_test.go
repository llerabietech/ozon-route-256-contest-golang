...
func TestRun(t *testing.T) {
	files, err := os.ReadDir("./who-is-meowing")
	...
	for _, file := range files {
		...
		fileTask, err := os.Open(fmt.Sprintf("who-is-meowing/%s", file.Name()))
		require.NoError(t, err)
		...
		t.Run(fmt.Sprintf("Test:%s", file.Name()), func(t *testing.T) {
			...
			expected, err := os.ReadFile(fmt.Sprintf("who-is-meowing/%s.a", file.Name()))
			require.NoError(t, err)
